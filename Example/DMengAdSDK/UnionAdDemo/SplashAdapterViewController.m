...
#import <DMengAdSDK/DMengSplashManager.h>
#import <DMengAdSDK/DMengAdManager.h>
...
@interface SplashAdapterViewController ()<UITableViewDelegate,UITableViewDataSource,DMengSplashManagerDelegate>
@property (nonatomic, strong) UITableView *listTable;
@property (nonatomic, copy) NSArray *titleArr;
@property (nonatomic, assign) bool isFull;
@property (nonatomic, strong) DMengSplashAdapter * splashAd;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[DMengAdManager shareInstance] initUnionSDKCompletion:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row==0) {
        ...
        _isFull = NO;
       [[DMengSplashManager new] loadSplashAdTemplateAdWithSlotID:@"118171711352274" adSize:CGSizeMake([UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height*0.75) completion:^(DMengSplashAdapter * splashAdapter) {
        ...
        _isFull = YES;
        [[DMengSplashManager new] loadSplashAdTemplateAdWithSlotID:@"118171711352274" adSize:CGSizeMake([UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)  completion:^(DMengSplashAdapter * splashAdapter) {
            self.splashAd = splashAdapter;
            self.splashAd.delegate = self;
            NSLog(@"当前返回的对象为%@",self.splashAd);
         }];
