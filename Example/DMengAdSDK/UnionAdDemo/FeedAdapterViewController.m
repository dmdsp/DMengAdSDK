...
#import <DMengAdSDK/DMengFeedManager.h>

static NSString *cellWithIdentifier = @"cellWithIdentifier";

@interface FeedAdapterViewController ()<UITableViewDelegate,UITableViewDataSource,DMengFeedManagerDelegate>
@property (nonatomic, strong) UITableView *listTable;
@property (nonatomic, copy) NSArray *titleArr;
@property (nonatomic, strong) DMengFeedAdapter * feedAd;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row==0) {
        [[DMengFeedManager new] loadFeedAdTemplateAdWithSlotID:@"118171711352246" popupViewHidden:NO completion:^(DMengFeedAdapter * _Nonnull feedAdapter) {
            self.feedAd = feedAdapter;
            self.feedAd.delegate = self;
        }];
    }else{
        [[DMengFeedManager new] loadFeedAdTemplateAdWithSlotID:@"118171711352246" popupViewHidden:YES completion:^(DMengFeedAdapter * _Nonnull feedAdapter) {
            self.feedAd = feedAdapter;
            self.feedAd.delegate = self;
        }];    }
}
#pragma  ---DMFeedAdDelegate
- (void)feedAdDidClick:(nonnull DMengFeedAdapter *)feedAd {
...
- (void)feedAdDidLoad:(nonnull DMengFeedAdapter *)feedAd {
...
- (void)feedAdDidRender:(nonnull DMengFeedAdapter *)feedAd {
...
- (void)feedAdDidShow:(nonnull DMengFeedAdapter *)feedAd {
