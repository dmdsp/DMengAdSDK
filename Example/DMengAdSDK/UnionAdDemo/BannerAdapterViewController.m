...
#import <DMengAdSDK/DMengBannerManager.h>
static NSString *cellWithIdentifier = @"cellWithIdentifier";

@interface BannerAdapterViewController ()<UITableViewDelegate,UITableViewDataSource,DMengBannerManagerDelegate>
@property (nonatomic, strong) UITableView *listTable;
@property (nonatomic, copy) NSArray *titleArr;
@property (nonatomic, strong) DMengBannerAdapter * bannerAd;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row==0) {
        [[DMengBannerManager new] loadBannerAdTemplateAdWithSlotID:@"118171711352334" popupViewHidden:NO completion:^(DMengBannerAdapter * _Nonnull bannerAdapter) {
            self.bannerAd = bannerAdapter;
            self.bannerAd.delegate = self;
        }];
    }else{
        [[DMengBannerManager new] loadBannerAdTemplateAdWithSlotID:@"118171711352334" popupViewHidden:YES completion:^(DMengBannerAdapter * _Nonnull bannerAdapter) {
            self.bannerAd = bannerAdapter;
            self.bannerAd.delegate = self;
        }];
    }
...
- (void)bannerAdDidClick:(nonnull DMengBannerAdapter *)bannerAd {
...
- (void)bannerAdDidLoad:(nonnull DMengBannerAdapter *)bannerAd {
...
- (void)bannerAdDidRender:(nonnull DMengBannerAdapter *)bannerAd {
...
- (void)bannerAdDidShow:(nonnull DMengBannerAdapter *)bannerAd {
